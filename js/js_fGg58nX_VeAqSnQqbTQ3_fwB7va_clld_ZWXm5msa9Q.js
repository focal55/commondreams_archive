...
(function ($) {
    ...
    Drupal.behaviors.headerBehavior = {
        attach: function (context, settings) {
            ...
            slimHeaderOriginalContent = $('#slim-display-content').html();
            ...
            for (var i = 0; i < continuousElements.length; i++) {
                ...
                new Waypoint({
                    ...
                    //enabled: false,
                    handler: function(direction) {
                        if ($('#slim-display-content').length) {

                            if (direction === 'down') {
                                var content = $(this.element).data('cta-slim-header');
                                $('#slim-display-content').html(content);
                            }
                            if (direction === 'up') {
                                $('#slim-display-content').html(slimHeaderOriginalContent);
                            }
                        }
                    }
                })
  ...
  Drupal.behaviors.slideDownFromTopCta = {
    attach: function (context, settings) {
      if (settings.ctas.slide_down_from_top) {
        ...
        var hide_banner = localStorage.getItem('commondreams.cta.hide_banner');
        if (!hide_banner) {
          ...
          if (localStorage.getItem('commondreams.cta.' + block_id) !== 'shown') {
            ...
            localStorage.setItem('commondreams.cta.' + block_id, 'shown');
          }

          // Already donated button.
          $(block_id).find(".hide-banner").click(function(e) {
            localStorage.setItem('commondreams.cta.hide_banner', 1);
            $(block_id).remove();
            e.preventDefault();
          });
